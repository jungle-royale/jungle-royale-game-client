        ...
        if (gameState.PlayerState != null && gameState.PlayerState.Count > 0)
        {
            ...
            foreach (var player in gameState.PlayerState)
            {
                Player newPlayer = new Player(
                    player.Id, player.X, player.Y, player.Dx, player.Dy, player.Health, player.MagicType, player.Angle, player.DashCoolTime, player.IsMoved, player.IsDashing, player.IsShooting
                );
